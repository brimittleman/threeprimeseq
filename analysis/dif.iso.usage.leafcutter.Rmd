...
```{r}
counts_melt_ENSG00000163632.8_r= counts_melt_ENSG00000163632.8 %>% group_by(variable) %>% mutate(sum=sum(value)) %>% ungroup(variable) %>%  mutate(ratio=value/sum)
...
```{r}
ggplot(counts_melt_ENSG00000163632.8_r, aes(x=bin, y=ratio, fill=fraction)) + geom_boxplot() + labs(title="Used polyA sites in ENSG00000163632.8 by fraction", y="Count ratio")
```

Do this for the counts_melt_ENSG00000182578.9
