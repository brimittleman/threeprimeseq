...
```{r}
summary(TXN_NormGene$TPM)
summary(TXN_NormGene$TPM+ .001)
ggplot(TXN_NormGene, aes(x=TPM + .001)) + geom_density(kernel="gaussian") + scale_x_log10()
```
```{r}
summary(TXN_NormGene$NormGenePeakCov)
ggplot(TXN_NormGene, aes(x=NormGenePeakCov)) + geom_density(kernel="gaussian")+ scale_x_log10()
```
